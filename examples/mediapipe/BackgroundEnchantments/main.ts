...
import { BlurRadius, createVonageMediaProcessorConnector, getVonageEventEmitter, isSupported, MediaProcessorConfig, setBackgroundOptions, setTrackExpectedRate } from '@vonage/ml-transformers'
...
async function main() {
  ...
  const backgroundPostProcessSelector: any = document.getElementById('postProcessSelector')
  let cameraSwitchChecked: boolean = false

  const MEDIA_ASSETS_URI: string = "https://vonage-background-enchantments-sample.s3.amazonaws.com/"
  ...
  async function updateCameraSwitch(isChecked: boolean) {
    ...
    setVonageMetadata({appId: 'test_app_id', sourceType: 'video'})
    if(!isChecked){
      backgroundPostProcessSelector.disabled = false
      await videoSource_.stopMediaProcessorConnector()
    }else{
      createVonageMediaProcessorConnector(getConfig(backgroundType)).then( connector => {
        getVonageEventEmitter().on('error', ( eventData => {
          console.log("error", eventData);
        }))
        getVonageEventEmitter().on('warn', (eventData => {
          console.log("warn", eventData);
        }))
        setTrackExpectedRate(30)
  ...
  if(githubButtonSelector){
    githubButtonSelector.addEventListener('click', () => {
      window.open("https://github.com/Vonage/vonage-media-transformers-samples/tree/main/examples/mediapipe/BackgroundEnchantments", '_blank')?.focus();
    })
  }
  ...
  backgroundPostProcessSelector.addEventListener('change', () => {
    ...
    if(cameraSwitchChecked){
      const backgroundType = backgroundPostProcessSelector.value
      setBackgroundOptions(getConfig(backgroundType)).then(() => {
